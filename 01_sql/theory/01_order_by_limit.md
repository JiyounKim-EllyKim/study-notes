...
## 5. 사용자 지정 순서 정렬 - FIELD

`FIELD()`를 `ORDER BY`와 함께 사용하면 원하는 순서를 직접 지정할 수 있다.
...
`FIELD(value, v1, v2, ...)`는 `value`가 목록에서 몇 번째에 위치하는지를 반환한다.

따라서 알파벳순이나 숫자순이 아닌 특정한 순서로 데이터를 정렬할 때 사용할 수 있다.

예를 들어 위 쿼리는 요일을 문자열의 가나다순이 아니라
`월요일 → 화요일 → ... → 일요일` 순서로 정렬한다.

> `FIELD()`는 MySQL에서 사용할 수 있는 함수이다.
